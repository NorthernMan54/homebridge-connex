      ...
      _getDevices.call(this, function(err, locations) {
        _getZones.call(this, function(err, zones) {
          _getProperties.call(this, function(err, zones) {
            debug("Devices", JSON.stringify(this.devices, null, 4));
...
function _getZone(zone, callback) {
  ...
  request({
    method: 'GET',
    url: PROPERTIES_URL + this.devices.dsn + '/data/' + zone + 'Name.json',
    timeout: 5000,
    strictSSL: false,
    headers: HEADER
    ...
    if (err || response.statusCode !== 200 || response.statusMessage !== "OK") {
      if (err) {
        ...
        debug("Error", PROPERTIES_URL + this.devices.dsn + '/data/' + zone + 'Name.json', JSON.stringify(HEADER));
...
function _getProperties(callback) {
  ...
  request({
    method: 'GET',
    url: PROPERTIES_URL + this.devices.dsn + '/properties.json',
    timeout: 30000,
    strictSSL: false,
    headers: HEADER
  }, function(err, response) {
...
function _parseProperties(properties) {
  properties.forEach(function(element) {
    ...
    if (element.property.name.substring(0, 1) === "Z") {
      ...
      this.devices.zones[zone][key] = element.property.value;
      this.devices.zones[zone][key + 'key'] = element.property.key;
      this.devices.zones[zone].CurrTemp = this.devices.CurrTemp;
    } else if (element.property.name === "CurrTemp") {
      this.devices.CurrTemp = element.property.value;
    }
