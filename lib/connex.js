...
connex.prototype.setTargetTemperature = function(zone, value, callback) {
  debug("setTargetTemperature %s ===> ", zone.name, zone);
  var body = {
    "datapoint": {
      "value": value * 10,
      "metadata": null
  ...
  debug("setTargetTemperature %s ===> ", zone.name, JSON.stringify(body));
