...
function connex(options, callback) {
  ...
  pollDevices.call(this, function(err, result) {
    ...
    setInterval(pollDevices.call(this, function(err, result) {
      if (!err) {
        debug("setInterval", result);
        devices = result;
      } else {
        debug("poll Error", err);
      }
    }), this._refresh * 1000).bind(this); // Poll every minute
    callback(err, result);
  }.bind(this));
  ...
  }), this._refresh * 1000); // Poll every minute
  */
...
function _getZone(devices, zone, callback) {
  ...
  if (!devices || !devices.AccessToken) {
...
function _getDevices(devices, callback) {
  ...
  HEADER = {
    ...
    'user-agent': 'AMAP/5.8.8 (iPad; iOS 13.1.3; Scale/2.00)',
    'accept-language': 'en-CA;q=1',
    'authorization': 'auth_token ' + devices.AccessToken
  };
  ...
  request({
    ...
    if (err || response.statusCode !== 200 || response.statusMessage !== "OK") {
      ...
      if (json) {
        ...
        device.zones = {};
        device.AccessToken = devices.AccessToken;
